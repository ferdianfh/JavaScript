...
if (typeof totalScore !== `number`) {
    console.log(`Please enter score with number!`);
} else if (Number.isNaN(totalScore)) {
    console.log(`Score is not complete. Please enter all score!`);
} else {
...
if (averageScore >= 90 && averageScore <= 100) {
    ...
    console.log(`I'm sorry you couldn't make it`);
}
...
gradeCalculator = (bahasa, english, maths, science) => {
    ...
    if (typeof subjects !== `number`) {
        return `Please enter your score with number!`;
    } else if (Number.isNaN(subjects)) {
        return `You must enter all subjects!`
    } else {
        if (averageScore >= 90 && averageScore <= 100) {
...
console.log(gradeCalculator(80, 84, 96, 77));